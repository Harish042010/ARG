...
import { getAchievements, saveAchievements, generateId, type Achievement } from '../../data/adminStore';
...
const AdminAchievements = () => {
  ...
  const save = () => {
    ...
    if (isEditMode) {
      updated = items.map(a => a.id === editing.id ? (editing as Achievement) : a);
      notify('✅ Achievement updated successfully');
    } else {
      const newItem: Achievement = { ...(editing as Achievement), id: generateId() };
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setEditing(p => ({ ...p, studentName: e.target.value }))}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 focus:outline-none"
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={editing.award ?? ''}
                    onChange={e => setEditing(p => ({ ...p, award: e.target.value }))}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 focus:outline-none"
              ...
              <div>
                ...
                <div className="flex gap-3 flex-wrap">
                  {(editing.photos ?? []).map((src, i) => (
                    <div key={i} className="relative w-20 h-20 rounded-xl overflow-hidden border border-gray-200 group">
                      ...
                      <button
                        ...
                        className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-5 h-5 text-xs font-bold flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                        ✕
                      </button>
                    </div>
                  ...
                  {(editing.photos ?? []).length < 3 && (
                    <button
                      ...
                      className="w-20 h-20 rounded-xl border-2 border-dashed border-blue-200 text-blue-400 hover:border-blue-400 hover:bg-blue-50 transition-colors flex flex-col items-center justify-center gap-1"
                    >
                ...
                <p className="text-xs text-gray-400 mt-1.5">
                  If no photos uploaded, sample images will show on the public page.
                </p>
              </div>
            </div>
